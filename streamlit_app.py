...
st.markdown(f"""
    ...
    /* Ajuste para que el login no se rompa */
    [data-testid="stForm"] {{
...
def check_password():
    ...
    if st.session_state.get("password_correct") is not True:
        # Centramos el login y lo hacemos compacto
        ...
        with c_l2:
            ...
            with st.form("login_form"):
                st.text_input("Usuario", key="username", placeholder="Tu legajo o mail")
        ...
        return False # Bloquea la ejecución del resto de la app
    ...
    with c1:
        ...
        if st.button("🚀 COMENZAR PROCESO", use_container_width=True):
            if not user or not pw or not texto_sucio:
                ...
                if not internos:
                    st.warning("No se detectaron internos válidos.")
                else:
                    ...
                    if bot.iniciar(user, pw):
                        ...
                        for i, interno in enumerate(internos):
                            ...
                            # Alertas en terminal según estado
