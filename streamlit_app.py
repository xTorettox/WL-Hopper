...
def check_password():
    ...
    if "password_correct" not in st.session_state:
        ...
        st.write("# 🚀 Bienvenido a WL Hopper")
        st.write("Por favor, identificate para acceder al sistema de descargas.")
...
# --- FLUJO PRINCIPAL ---
if check_password():
    # Solo si el password es correcto se ejecuta el resto de la app
    
    # Botón de Cerrar Sesión en la barra lateral
    st.sidebar.button("Cerrar Sesión", on_click=lambda: st.session_state.clear())

    # --- ESTILOS CSS (Tu configuración actual) ---
    ...
    with c1:
        st.info("📌 Paso 1: Configurar credenciales y archivos")
        user = st.text_input("Usuario Worklift", value="", placeholder="ejemplo@sullair.com.ar")
        pw = st.text_input("Contraseña", type="password")
        
        file = st.file_uploader("Subir Excel de Internos", type=["xlsx", "csv"])
        
        st.divider()
        st.info("⚙️ Paso 2: Opciones de descarga")
        b_cert = st.checkbox("Descargar Certificados", value=True)
        b_inf = st.checkbox("Descargar Informes", value=True)
        
        if st.button("🚀 COMENZAR PROCESO", use_container_width=True):
            if not user or not pw or not file:
                st.error("Faltan datos obligatorios.")
            else:
                asegurar_carpeta("descargas_temp")
                internos = extraer_internos(file)
                ...
                if bot.iniciar(user, pw):
                    ...
                    st.session_state.log_history.append("❌ Error de Login.")
